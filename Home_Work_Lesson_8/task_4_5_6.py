...
class Printer(OfficeEquipment):
    ...
    def __str__(self):
        return f'Тип: {self.type}, Инв.номер (id): {self.id}, Стоимость: ' \
               f'{self.price},' \
...
class InitStorage:
    ...
    def __get_type(self):
        while True:
            try:
                print(
                    'Выберите тип техники, которую следует поместить на склад:')
    ...
    def __make_obj(self):
        ...
        for i in range(self.__qty):
            price, place, param = self.__price[i], self.__place[i], \
                                  self.__inner_param[i]
